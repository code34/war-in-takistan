	...
	if !(isNull player) then
	{
		...
		if (_count_liste_vehicules > 0) then
		{
			...
			for [{_i = 0}, {_i < _count_liste_vehicules}, {_i = _i + 1}] do
			{
				...
				{
					_objet addAction [("<t color=""#dddd00"">" + STR_R3F_ARTY_action_ouvrir_dlg_SM + "</t>"), "extern\R3F_ARTY_AND_LOG\R3F_ARTY\poste_commandement\ouvrir_dlg_saisie_mission.sqf", nil, 6, false, true, "", "vehicle player == _target"];
				};
				
				// Si l'objet doit mettre à disposition un calculateur d'artillerie depuis l'intérieur
				if ({_objet isKindOf _x} count R3F_ARTY_CFG_calculateur_externe > 0) then
				{
					_objet addAction [("<t color=""#dddd00"">" + STR_R3F_ARTY_action_ouvrir_dlg_SM + "</t>"), "extern\R3F_ARTY_AND_LOG\R3F_ARTY\poste_commandement\ouvrir_dlg_saisie_mission.sqf", nil, 6, true, true, "", "vehicle player == player"];
				};
