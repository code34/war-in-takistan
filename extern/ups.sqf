...
if (_issoldier) then {
	switch (side _npc) do {
		...
		case east:
			{ _friends=_friends+WC_KRON_AllEast+WC_KRON_AllRes; _enemies=_enemies+WC_KRON_AllWest; _sharedenemy=1; };
		case resistance:
			{ _friends=_friends+WC_KRON_AllEast+WC_KRON_AllRes; _enemies=_enemies+WC_KRON_AllWest; _sharedenemy=2; };
	};
	{
...
if (_rangeX==0) exitWith {
	...
	diag_log format ["WARCONTEXT: UPS - %1 AREA marker doesn t exist ", _areaname];
};
